...
class UpscalingService {
    ...
    public function handle_webhook() {
        ...
        if ( empty( $webhook_secret ) ) {
            WebhookManager::send_webhook_error( 'SMI UpscalingService: Webhook secret not configured', 500 );
        }
        ...
        if ( empty( $auth_header ) || $auth_header !== $webhook_secret ) {
            WebhookManager::send_webhook_error( 'SMI UpscalingService: Invalid authentication', 401 );
        }
        ...
        if ( ! isset( $data['jobId'], $data['status'] ) ) {
            WebhookManager::send_webhook_error( 'Missing required fields: jobId, status', 400 );
        }
        
        // Validate jobId format (should be UUID-like)
        if ( ! preg_match( '/^[a-f0-9\-]{36}$/i', $data['jobId'] ) ) {
            WebhookManager::send_webhook_error( 'Invalid jobId format', 400 );
        }
    ...
    private function handle_upsampler_webhook( $webhook_data ) {
        $upsampler_job_id = $webhook_data['jobId'];
        ...
        switch ( $status ) {
            ...
            case 'processing':
                ...
                JobManager::update_job_status( $job->job_id, 'processing' );
                break;
    ...
    private function handle_upscaling_completed( $job, $webhook_data ) {
        $upscaled_url = $webhook_data['outputUrl'] ?? null;
        
        if ( ! $upscaled_url ) {
            error_log( 'SMI UpscalingService: No output URL in completed webhook for job: ' . $job->job_id );
            JobManager::update_job_status( $job->job_id, 'failed', array(
                'failure_reason' => 'No output URL provided'
            ) );
            return;
        }
        
        // Update job with completed status and output URL
        $update_result = JobManager::update_job_status( $job->job_id, 'completed', array(
